...
if __name__ == "__main__":
    ...
    def train(Epoch):
        ...
        #for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            progress_bar.set_description(f"Train Loss: {train_loss/(batch_idx+1):.3f} | Acc: {100*correct/total:.3f}%% ({correct:d}/{total:d})")
    ...
    def test(epoch, train_set=False):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (inputs, targets) in enumerate(progress_bar):
                ...
                if train_set:
                    progress_bar.set_description(f"Train Loss: {loss/(batch_idx+1):.3f} | Acc: {100*correct/total:.3f}%% ({correct:d}/{total:d})")
                else:
                    progress_bar.set_description(f"Test Loss: {loss/(batch_idx+1):.3f} | Acc: {100*correct/total:.3f}%% ({correct:d}/{total:d})")
    ...
    if folder_prefix is not None:
        ...
        folder = f"{folder_prefix} - {lr} - {batch_size} - {momentum} - {weight_decay}" + " - BP"*(use_backprop == True)  + f" - {info}"*(info != "")
    else:
        folder = None
    ...
    def train_fb():
        ...
        for i in range(3):
            ...
            if sum(fb_weight != 0) > 0:
                corr_percent = 100*sum((sign(fb_weight.T) == sign(weight)) & (fb_weight.T != 0))/sum(fb_weight.T != 0)
            else:
                corr_percent = 0
